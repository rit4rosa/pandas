...
class _MergeOperation:
    ...
    def _maybe_add_join_keys(
        ...
        # Ensure 'self.left_join_keys' contains valid objects (ndarray, Series, or Index)
        assert all(isinstance(x, (np.ndarray, Series, Index)) for x in self.left_join_keys)

        keys = zip(self.join_names, self.left_on, self.right_on)
        for i, (name, lname, rname) in enumerate(keys):
            # Skip if there's no need to fill (e.g., no join keys involved or no columns to create)
            if not _should_fill(lname, rname):
                continue

            take_left, take_right = None, None

            # Determine if left/right join keys need to be taken from self.left/self.right
            # This part ensures that if a column named 'name' already exists in 'result',
            # we might need to fetch the original values for type consistency or missing value handling.
            if name in result:
                ...
                # If 'name' is not already in 'result', default to taking values from join_keys
                take_left = self.left_join_keys[i]
                take_right = self.right_join_keys[i]
            ...
            if take_left is not None or take_right is not None:
                ...
                if self.coalesce_keys:
                    ...
                    # Where left_indexer is -1 (meaning the row came from the right DataFrame
                    # and has no match on the left), use the corresponding right-side value.
                    ...
                    # Workaround for datetime types that might incorrectly become 'Object'
                    if (
                        lvals.dtype.kind == "M"
                    ...
                    # If 'name' (the join key, e.g., 'id') already exists as a column label
                    # in the 'result' DataFrame, we need to UPDATE that column.
                    # In a coalesced merge, the join key column is always expected to exist
                    # as a label.
                ...
                else: # This block handles coalesce_keys=False (separate key columns)
                    left_name = lname
                    # Generate the right column name, adding a suffix if names are identical
                    right_name = f"{rname}{self.suffixes[1]}" if rname != lname else f"{lname}{self.suffixes[1]}"

                    # Cast to float if NaNs are present, to ensure numerical compatibility
                    ...
                    if right_name not in result.columns:
                        if result._is_label_reference(right_name):
                            ...
                            try:
                                # Insert right column next to the left column for better readability
                                insert_pos = result.columns.get_loc(left_name) + 1
                                result.insert(insert_pos, right_name, rvals)
                            except KeyError:
                                # Fallback if left_name isn't found for insertion positioning
